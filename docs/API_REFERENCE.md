...
### Direct-only liste + varsayilan 302 yayin (kota korumasi)

- `GET /get.php` SADECE `?direct=1` (veya `?redirect=1`) ile calisir;
  parametre yoksa `403` doner. `direct=1` ile istemci upstream `get.php`
...
- `/live/...`, `/movie/...`, `/series/...`, `/timeshift/...` istekleri
  **varsayilan** olarak upstream adresine 302 ile yonlendirilir: oturum
  sayimi ve istatistik yapilir, video baytlari bu sunucudan gecmez (kota
  korunur). Boylece oynaticilarin kendilerinin kurdugu sade linkler de
  calisir (Xtream girisiyle baglanan TiviMate/APK bozulmaz).
...
- Paylasim misafiri (`/token/auth/...`) her zaman proxy ile izler, upstream
  kimligi disari sizmaz.
- `player_api.php` (kimlik + katalog) ve `xmltv.php` (panel rehberi) bu
  ...
  ekler.
- Kisi basi agir uc korumasi, GUNLUK kayar pencere (adminler muaftir):
  `get.php` gunde 100 (misafir 2); panel `xmltv.php` gunde 10, upstream
  302 yolu gunde 100 (misafir 2); kategorisiz (tam) `get_*_streams` gunde
  10; kategorili alt kirilimlar gunde 400; kategori listeleri gunde 400.
  Bayt tasimayan 302'ler kota yemez; MB tasiyan yollarda limit sikidir.
  Reddedilen (401/403) istekler hak yemez. Asimda `429 + Retry-After`
  doner; 10sn'de liste soran donguler gunun geri kalaninda 429 yer.
