...
async function addDirectM3u() {
  ...
  const btn = document.getElementById('btn-direct-m3u-add');
  const url = input?.value?.trim();
  if (!url) { showToast('M3U linkini gir', 'warning'); return; }
  if (!selectedUserId) { showToast('Önce kullanıcı seç', 'warning'); return; }
  try {
    ...
    const res = await fetchJSON('/api/providers/direct-m3u', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ url, user_id: selectedUserId })
    });
    ...
    loadProviders();
    loadLocalM3u();
    // Atanmış listesini yenile (otomatik son haliyle açılır)
